...
def simulate_orders(N=700000, start_date="2014-01-01", end_date=None):
    ...
    unique_cust_ids = np.random.randint(
        1000000, 9999999, cust_counts.sum(), dtype=np.int64
    )
    cust_id_index = list(
        chain(
            *[
    ...
    out = pd.DataFrame({"Customer ID": customer_id})

    # ------- Day
    real_N = out.shape[0]
    out["Day"] = (
        pd.Series(random_dates(start_date, end_date, real_N)).sort_values().values
    )
    ...
    out["Ordered quantity"] = (
        np.round(np.interp(np.random.rand(real_N), quantiles, quantity_per_order_qs))
        * out["orders"]
    )
    out["Ordered quantity"] = out["Ordered quantity"].astype(int)

    # ------- Gross sales: float (always non-negative)
    price_per_item_qs = np.array(
        [
            0.0,
            4.95,
            6.95,
    ...
    price_per_item = np.interp(np.random.rand(real_N), quantiles, price_per_item_qs)
    ...
    out["Discounts"] = (
        np.interp(np.random.rand(real_N), quantiles, non_neg_discount_qs) * discounted
    )
    ...
    out["Tax"] = (
        np.interp(np.random.rand(real_N), quantiles, tax_rate_qs)
        * out["Gross sales"]
        * taxed
        / 100
    )
    ...
    out["Shipping"] = (
        np.interp(np.random.rand(real_N), quantiles, non_zero_shipping_per_order_qs)
        * shipping
    )
    ...
    out["Net sales"] = out["Gross sales"] + out["Discounts"] + out["Returns"]
